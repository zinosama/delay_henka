  ...
  RSpec.describe ApplyActionsWorker do
    ...
    describe '#perform' do
      ...
      it 'does not execute action outside of a given time zone' do
        ScheduledAction.schedule(record: actionable, by_email: 'tester@chowbus.com', method_name: 'destroy', schedule_at: 1.hour.ago, time_zone: "Eastern Time (US & Canada)")
        scheduled_action = ScheduledAction.last
      ...
      it 'does not execute action if schedule_at is in the future' do
        ScheduledAction.schedule(record: actionable, by_email: 'tester@chowbus.com', method_name: 'destroy', schedule_at: 1.hour.from_now, time_zone: time_zone)
        scheduled_action = ScheduledAction.last
